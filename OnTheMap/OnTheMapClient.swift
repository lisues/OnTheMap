...
class OnTheMapClient: NSObject {
    ...
    func taskRequest(_ requestType: RequestType,_ method: String, addHeaderField: [String:String]?, setHeaderField: [String:String]?, parameters: [String:AnyObject], jsonBody: String?, completionHandlerForRequest: @escaping (_ data: Data?, _ error: NSError?) -> Void) -> URLSessionDataTask {

        let urlPath = method+escapedParameters(parameters)
        let request = NSMutableURLRequest(url: URL(string:urlPath)!)
        if let methodType=getHttpMethod(requestType:requestType) {
    ...
    private func getHttpMethod(requestType: RequestType) -> String? {
        switch requestType {
            case .udacityPOST, .parsePOST:
                return "POST"
            case .udacityDELETE:
                return "DELETE"
            case .parsePUT:
